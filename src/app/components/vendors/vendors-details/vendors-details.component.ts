...
export class VendorsDetailsComponent implements OnInit {
  ...
  constructor(
    public dialogRef: MatDialogRef<VendorsDetailsComponent>,
  ...
  ngOnInit(): void {
    ...
    this.http.post<any>(
      ...
      next: (res) => {
        ...
        if (res.status === 1) {
          this.vendor = { ...this.vendor, ...res.data.user }; // Merge API data with default structure
          if (this.vendor.userRoleId === undefined) {
